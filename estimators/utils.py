...
def _get_env_float(name: str, default: float) -> float:
    """安全读取环境变量中的浮点数，不存在或非法时返回默认值"""
    try:
        return float(os.environ.get(name, default))
    except Exception:
        return default

def get_PGAE_design(df: pd.DataFrame, X: List[str], F: str, Y: str, gamma: float,
                    min_var_threshold: Optional[float] = None,
                    clip_low: Optional[float] = None,
                    clip_high: Optional[float] = None) -> pd.DataFrame:
    """
    ...
    # 添加数值稳定性与可调裁剪（不改变指数部分，保持算法本质）
    # - PGAE_MIN_VAR_THRESH: 下界阈值，默认1e-4
    # - PGAE_CLIP_LOW / PGAE_CLIP_HIGH: 概率裁剪范围，默认[0.05, 0.95]
    # 允许通过参数覆盖（否则从环境或默认读取）
    min_var_threshold = min_var_threshold if min_var_threshold is not None else _get_env_float('PGAE_MIN_VAR_THRESH', 0.0001)
    clip_low = clip_low if clip_low is not None else _get_env_float('PGAE_CLIP_LOW', 0.05)
    clip_high = clip_high if clip_high is not None else _get_env_float('PGAE_CLIP_HIGH', 0.95)
    ...
    # 与原实现保持一致：sample_pmf ∝ true_pmf * sqrt(var_of_cond_mean)
    ...
    # 与原实现保持一致：exp_prob ∝ sqrt(mean_of_cond_var / var_of_cond_mean)
    ratio = group_stats['mean_of_cond_var'] / group_stats['var_of_cond_mean']
    group_stats['exp_prob'] = np.sqrt(ratio)
    ...
    # 更鲁棒的概率clipping - 可调（不改变指数结构）
    group_stats['exp_prob'] = np.clip(group_stats['exp_prob'], clip_low, clip_high)
    # 与原实现保持一致：accept_prob ∝ sqrt(var_of_cond_mean)，并归一化到[0,1]
    accept_score = np.sqrt(group_stats['var_of_cond_mean'])
    max_score = accept_score.max()
    group_stats['accept_prob'] = accept_score / max_score if max_score > 0 else 1.0
    group_stats = group_stats.drop(columns=['true_pmf', 'cnt'])

    return group_stats

def PGAE_est_ci_param(PGAE_df: pd.DataFrame, X: List[str], F: str, Y: str,
                      alpha: float = 0.95, K: int = 3,
                      n_estimators_mu: Optional[int] = None,
                      n_estimators_tau: Optional[int] = None,
                      max_depth: Optional[int] = None,
                      min_samples_split: Optional[int] = None,
                      min_samples_leaf: Optional[int] = None) -> Tuple[float, float, float]:
    """
    与 PGAE_est_ci 等价，但允许注入随机森林与折数参数。
    未提供的参数将退回到 PGAE_est_ci 的默认取值。
    """
    from sklearn.ensemble import RandomForestRegressor
    # 默认值与 PGAE_est_ci 对齐
    n_estimators_mu = 50 if n_estimators_mu is None else n_estimators_mu
    n_estimators_tau = 100 if n_estimators_tau is None else n_estimators_tau
    min_samples_split = 5 if min_samples_split is None else min_samples_split
    # 与 tune_internal_parameters 默认保持一致：min_samples_leaf=2
    min_samples_leaf = 2 if min_samples_leaf is None else min_samples_leaf
    ...
    for i in range(K):
        ...
        model_mu = RandomForestRegressor(n_estimators=n_estimators_mu, max_depth=max_depth,
                                         min_samples_split=min_samples_split, min_samples_leaf=min_samples_leaf,
                                         random_state=42)
        model_mu.fit(PGAE_df.loc[fold2, X], PGAE_df.loc[fold2, Y])
        PGAE_df.loc[fold1+unlabeled_fold1, 'mu'] = model_mu.predict(PGAE_df.loc[fold1+unlabeled_fold1, X])

        model_tau = RandomForestRegressor(n_estimators=n_estimators_tau, max_depth=max_depth,
                                          min_samples_split=min_samples_split, min_samples_leaf=min_samples_leaf,
                                          random_state=42)
        model_tau.fit(PGAE_df.loc[fold2, X+[F]], PGAE_df.loc[fold2, Y])
        PGAE_df.loc[fold1+unlabeled_fold1, 'tau'] = model_tau.predict(PGAE_df.loc[fold1+unlabeled_fold1, X+[F]])
