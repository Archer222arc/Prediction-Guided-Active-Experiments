#!/usr/bin/env python3
"""
Active Statistical Inference Estimator
主动统计推断估计器
"""
...
class ActiveInferenceEstimator:
    """主动统计推断估计器类"""
    ...
    def __init__(self, X: List[str], F: str, Y: str, gamma: float = 0.5):
        ...
        logger.info(f"主动统计推断估计器初始化: X={X}, F={F}, Y={Y}, gamma={gamma}")
    ...
    def prepare_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 计算预测误差
        df_clean['pred_error'] = np.abs(df_clean[self.Y] - df_clean[self.F])
        
        # 按协变量分组计算实验概率
        summary = df_clean.groupby(self.X)['pred_error'].agg(
            exp_prob='mean',
            pmf='count'
        ).reset_index()
        
        # 标准化PMF
        summary['pmf'] = summary['pmf'] / summary['pmf'].sum()
        
        # 标准化实验概率
        exp_prob_weighted = (summary['exp_prob'] * summary['pmf']).sum()
        if exp_prob_weighted > 0:
            summary['exp_prob'] = summary['exp_prob'] / exp_prob_weighted * self.gamma
        else:
            summary['exp_prob'] = self.gamma / len(summary)
        
        # 合并回原数据
        df_clean = df_clean.merge(summary, on=self.X, how='left')
    ...
    def run_single_experiment(self, df: pd.DataFrame, n_true_labels: int = 500, 
        ...
        while cnt_true < n_true_labels:
            # 随机采样
            sampled_df = df_work.sample(n=n_true_labels, replace=True)
            ...
            active_df.append(sampled_df)
        
        # 合并所有采样数据
        active_df = pd.concat(active_df, ignore_index=True)
        
        # 截断到目标标签数量
        active_df['cum_sum'] = active_df['true_label'].cumsum()
        cutoff_index = active_df[active_df['cum_sum'] == n_true_labels].index[0]
        active_df = active_df.iloc[:cutoff_index + 1]
        
        # 计算估计值（重要性加权估计）
        # tau = E[F + (Y - F) * I / p]，其中I是标记指示器，p是实验概率
        weighted_estimates = (
            active_df[self.F] + 
            (active_df[self.Y] - active_df[self.F]) * 
            active_df['true_label'] / active_df['exp_prob']
        )
        
        tau_active = weighted_estimates.mean()
        var_active = weighted_estimates.var()
        
        # 计算置信区间
        z_alpha = norm.ppf(1 - (1 - alpha) / 2)
        margin_of_error = z_alpha * np.sqrt(var_active / len(active_df))
        
        l_ci = tau_active - margin_of_error
        h_ci = tau_active + margin_of_error
        
        return tau_active, l_ci, h_ci
    ...
    def run_experiments(self, df: pd.DataFrame, n_experiments: int = 1000, 
        ...
        Returns:
            实验结果字典
        """
        logger.info(f"开始主动统计推断实验: {n_experiments}次实验, 每次{n_true_labels}个标签")
        if use_concurrent:
        ...
        results = summary_results(tau_results, l_ci_results, h_ci_results, 
                                true_value, "Active Statistical Inference")
        ...
        logger.info(f"主动统计推断实验完成，耗时: {results['execution_time']:.2f}秒")
    ...
    def _run_sequential_experiments(self, df_prepared: pd.DataFrame, n_experiments: int,
        ...
        for i in tqdm(range(n_experiments), desc="主动推断实验进行中"):
    ...
    def _run_concurrent_experiments(self, df_prepared: pd.DataFrame, n_experiments: int,
        ...
        for i in range(n_experiments):
            exp_seed = None if seed is None else seed + i
            task_args.append((df_prepared, self.X, self.F, self.Y, self.gamma, n_true_labels, alpha, exp_seed))
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            # 提交任务
            future_to_index = {executor.submit(_run_single_active_experiment, *args): i 
                             for i, args in enumerate(task_args)}
            
            # 收集结果
            for future in tqdm(as_completed(future_to_index), total=n_experiments, desc="主动推断并发实验"):
    ...
    def save_results(self, results: Dict, filename: str = None) -> str:
        ...
        if filename is None:
            ...
            filename = f'active_inference_results_{timestamp}.json'
        ...
        logger.info(f"主动统计推断结果已保存: {filename}")
        return filename

# 独立的worker函数，用于并发执行
def _run_single_active_experiment(df_prepared: pd.DataFrame, X: List[str], F: str, Y: str,
                                 gamma: float, n_true_labels: int, alpha: float, 
                                 seed: Optional[int]) -> Tuple[float, float, float]:
    """
    运行单个主动统计推断实验的worker函数
    ...
    try:
        ...
        # 计算估计值 - 按照notebook公式
        tau_active = (active_df[F] + (active_df[Y] - active_df[F]) * active_df['true_label'] / active_df['exp_prob']).mean()
        var_active = (active_df[F] + (active_df[Y] - active_df[F]) * active_df['true_label'] / active_df['exp_prob']).var()
        
        # 计算置信区间
        coef = norm.ppf(1 - (1 - alpha) / 2)
        l_ci = tau_active - coef * np.sqrt(var_active / len(active_df))
        h_ci = tau_active + coef * np.sqrt(var_active / len(active_df))
...
def main():
    ...
    parser = argparse.ArgumentParser(description='主动统计推断估计器')
    ...
    # 初始化估计器
    estimator = ActiveInferenceEstimator(
        X=config['X'],
        F=config['F'],
        Y=config['Y'],
        gamma=args.gamma
    ...
    if output_file is None:
        timestamp = time.strftime("%Y%m%d_%H%M%S")
        output_file = f'active_inference_{args.target.lower()}_{timestamp}.json'
